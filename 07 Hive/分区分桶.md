...
(1)抽样更高效，因为在处理大规模的数据集时，在开发、测试阶段将所有的数据全部处理一遍可能不太现实，这时抽样就必不可少。
    ...
    抽样：
        SELECT * FROM table_name TABLESAMPLE(n PERCENT)； 就是针对n%进行取样
    ...
    有了桶之后呢？
        SELECT * FROM film TABLESAMPLE(BUCKET x OUTOF y)
            ...
            假设桶数是100，y=25时抽取(100/25) = 4个bucket数据;
            当y=200的时候(100/200) = 0.5个bucket的数据
...
在建立桶之前，需要设置 `set hive.enforce.bucketing=true;`，使得 hive 能识别桶。
