...
由于 HBase 内部将数据存储为 KeyValue 对，因此 KeyValue Metadata Filters 将评估一行的 keys 是否存在(即ColumnFamily:Column qualifiers)，而不是前一节的值。
...
ColumnPrefixFilter 在每行和涉及的每个列族中，查找与前缀匹配的第一个列。
...
MultipleColumnPrefixFilter 的行为类似于 ColumnPrefixFilter，但允许指定多个前缀。
...
与 ColumnPrefixFilter 一样，MultipleColumnPrefixFilter 高效地向前查找匹配最低前缀的第一列，并且还查找前缀之间的列的过去范围。它可以用来有效地从非常宽的行中获取不连续的列集。

> Example: Find all columns in a row and family that start with "abc" or "xyz"

示例：查找一行中以“abc”或“xyz”开头的所有列和族
...
ColumnRangeFilter 可以为每个涉及的列族查找第一个匹配的列。【？？？】

它可以用来获得非常宽的行的列的“切片”。

【在指定了列族范围和行范围，取一定范围内的列】
