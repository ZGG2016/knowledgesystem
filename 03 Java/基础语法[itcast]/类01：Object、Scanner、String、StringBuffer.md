# 类01：Object、Scanner、String、StringBuffer
...
返回此 Object 的运行时 类（返回对象的字节码文件对象）
		...
		Class类的方法：
			public String getName()：以 String 的形式返回此 Class 对象所表示的
			                         实体(接口、数组对象、基本类型、void类)名称。
```
...
返回该对象的字符串表示。
	...
	这个结构信息的组成是没有任何意义的。所以，建议所有子类都重写该方法。
	怎么重写呢?
		把该类的所有成员变量值组成返回即可。
...
public int nextInt() 获取下一个int类型的输入项

public String nextLine() 获取下一个String类型的值
